...
local SYSTEM_PROMPT = [[You are running inside Neovim through pi-background.nvim. The user sent a request with file context. Complete the task directly; do not ask for clarification unless absolutely necessary.]]

local EMPTY_FILE_NOTE = [[NOTE: This file is currently empty. Create or populate it directly if needed.]]
...
function M.get_buffer_context(bufnr, config)
  ...
  if did_trim_lines or did_trim_bytes then
    parts[#parts + 1] = string.format('NOTE: Context was trimmed for speed (max_lines=%d, max_bytes=%d).', config.max_context_lines, config.max_context_bytes)
  end
...
function M.get_visual_context(bufnr, config)
  ...
  if nearby_trimmed or selected_trimmed then
    parts[#parts + 1] = string.format('NOTE: Selection context was trimmed for speed (max_bytes=%d).', config.max_context_bytes)
  end
